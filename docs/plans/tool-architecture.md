...
struct ToolContext: Sendable {
    let workspace: Workspace          // resolved security-scoped folder URLs the user granted
...
All file tools operate only inside `Workspace` — folders the user granted via
`NSOpenPanel`, persisted as security-scoped bookmarks (survives relaunch; needed
regardless of app-sandbox status because it *is* the user-consent model). Every path
argument is canonicalized (`URL.standardizedFileURL`, resolve symlinks) and verified
inside a granted root before any I/O; escapes return a "path is outside your
approved folders" error. Names are model-facing; the UI shows friendly verbs, never
these identifiers.
...
range (≤20); .docx/.xlsx/.pptx → not v1: report the type honestly and punt (open
question 5 — office files are our users' world, but each format is real work).
...
directories only inside the workspace. Overwrite requires the path in the
...
`url`, optional `prompt` (ignored in v1 — no extraction model, see open Q3).
...
**`web_search`** — deliberately *not* a built-in.
There is no neutral local implementation — search is a backend service. Two honest
paths, not exclusive: (a) provider-hosted search tools (Anthropic, OpenAI, Gemini
grounding) exposed through adapters where they exist — FR-060 says exposing these
exclusives is right; (b) a neutral search API (Brave, Exa) needing its own key —
new product surface. Toni's call: open question 2.
...
- **Increment 5** (first tools, tested): Workspace + bookmarks; the six file tools;
  ...
  a live run in the app (DOD requires exercised, not inferred). The first
  `.writesWorkspace` invocation forces the minimal approval gate — whatever form the
  then-current requirements give it; that conversation happens at that DOR, since
  approvals-beyond-effect-class are explicitly "later".
...
## 6. Open questions (decisions this plan does not make)

1. **Approval UX for `.writesWorkspace` in increment 5.** "Specific tool approvals
   will come later" — but the first write needs *some* gate. Minimal per-task
   consent? Per-folder standing grant? Toni decides at that DOR.
2. **Web search backend.** Provider-hosted only (free, uneven coverage, exposes
   exclusives), neutral API key (Brave/Exa — new cost + signup), both, or neither
   in v1.
3. **`fetch_url` extraction model.** Claude Code burns a cheap-model call per fetch
   to keep pages out of the main window. Neutral equivalent = "use the cheapest
   model of the active provider" — real per-fetch cost. V1 returns paged markdown
   instead; revisit when fetch-heavy tasks appear.
4. **Bundled ripgrep** if native search proves slow (measure first — see
   `search_files`).
5. **Office formats** (.docx/.xlsx/.pptx) in `read_file` — almost certainly the
   actual substance of "work" for our users, and each is a real parser (or a
   dependency). Needs its own scoping conversation.
